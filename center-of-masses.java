...
public class center-of-masses
{
	...
	public static void main(String[] args) throws IOException{
		...
		while(points>2)
		{
				...
				{
					mult=(convex.get(i).x*convex.get((i+1)%convex.size()).y)-(convex.get((i+1)%convex.size()).x*convex.get(i).y);
					xsum+=((convex.get(i).x+convex.get((i+1)%convex.size()).x)*mult);
					ysum+=((convex.get(i).y+convex.get((i+1)%convex.size()).y)*mult);
					a+=mult;
				}
				a=a/2;
				xsum=xsum/6/a;
				ysum=ysum/6/a;
				sb.append(df.format(xsum)).append(" ").append(df.format(ysum)).append("\n");
