		...
		while((newline=stdin.readLine())!=null)
		{
			...
			if(points!=0)
			{
				...
				for(int i=1;i<convex.size();i++)
				{
					...
					{
						maxes[i-1]=Math.max(maxes[i-1],height(convex.get(i-1),convex.get(i),convex.get(j)));
					}
					if(maxes[i-1]<min)
					{
						min = maxes[i-1];
					...
					for(int j=0;j<convex.size();j++)
					{
						maxes[convex.size()-1]=Math.max(maxes[convex.size()-1],height(convex.get(convex.size()-1),convex.get(0),convex.get(j)));
					}

					if(maxes[convex.size()-1]<min)
					{
						min = maxes[convex.size()-1];
					}
					sb.append("Case ").append(caser).append(": ").append(df.format(Math.ceil(min*100)/100)).append("\n");
					caser++;
			}
		}
